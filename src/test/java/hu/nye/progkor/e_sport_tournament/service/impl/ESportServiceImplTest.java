...
import hu.nye.progkor.e_sport_tournament.model.Game;
import hu.nye.progkor.e_sport_tournament.model.Match;
import hu.nye.progkor.e_sport_tournament.model.exception.NotFoundException;
...
class ESportServiceImplTest {
  ...
  private static final Match LEAGUE_ESPORT_1 = new Match(2L, "2022:05:15", "DFM", "T1", 0, 1, Game.LeagueOfLegends);
  private static final Match LEAGUE_ESPORT_2 = new Match(1L, "2022:5:15", "AZE", "SGB", 0, 1, Game.LeagueOfLegends);
  ...
  void deleteMatchShouldDeleteMatchWhenGivenIdOfMatch() {
    //given
    final List<Match> expectedMatches = List.of(LEAGUE_ESPORT_2);
    //when
    underTest.deleteMatch(LEAGUE_ESPORT_1.getId());
    final List<Match> actual = underTest.getAllMatches();
    //then
